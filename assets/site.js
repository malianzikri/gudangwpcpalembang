...
const menu=$('.menu'), links=$('.links'); if(menu&&links){menu.addEventListener('click',()=>{const open=links.classList.toggle('open');menu.setAttribute('aria-expanded',String(open))});$$('.links a').forEach(a=>a.addEventListener('click',()=>{links.classList.remove('open');menu?.setAttribute('aria-expanded','false')}))}
...
if(form){
  form.addEventListener('submit',e=>{e.preventDefault();const w=parseFloat($('#width').value),h=parseFloat($('#height').value),areaName=$('#areaName').value.trim()||'Area pemasangan';if(!w||!h||w<=0||h<=0)return;
    const strips=Math.ceil(w/.16);let pieces;if(h<=3){const perPiece=Math.max(1,Math.floor(3/h));pieces=Math.ceil(strips/perPiece)}else{pieces=strips*Math.ceil(h/3)}pieces=Math.ceil(pieces*1.05);const area=(w*h).toFixed(2);const material=pieces*75000;const jasa=Math.round(w*h*60000);$('#calcResult').innerHTML=`<strong>± ${pieces} keping WPC</strong><div>Luas area ${area} m² • estimasi material mulai <b>${rupiah(material)}</b></div><small>Estimasi awal dengan lebar efektif 16 cm, panjang WPC 3 m, dan buffer ±5%. Potongan dapat dioptimalkan saat final quotation.</small>`;$('#sendCalc').hidden=false;$('#sendCalc').dataset.text=`Halo Gudang WPC & PVC Palembang, saya mau konsultasi WPC.\n\nArea: ${areaName}\nUkuran: ${w} m × ${h} m (${area} m²)\nEstimasi website: ±${pieces} keping WPC.\n\nMohon dicek kebutuhan final dan motif yang ready.`;track('WPC_Calculator',{area:parseFloat(area),pieces})
  });
